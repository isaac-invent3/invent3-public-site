...
import { AddIcon, CloseIcon } from '~/lib/components/CustomIcons';
...
const Approvers = ({ data, shouldEdit = true }: ApproversProps) => {
  ...
  return (
    <>
      ...
      >
        <SectionWrapper
          title={`Level ${data?.levelNumber}`}
          subtitle="This is the level of approval in the workflow."
        ...
        >
          <VStack width="full" alignItems="flex-start">
            <FieldArray name={`levels.${data.levelNumber - 1}.approvers`}>
              {({ insert, remove, form, push }) => {
                return (
                  <Flex gap="16px" flexWrap="wrap">
                    {form.values.levels[data.levelNumber - 1]?.approvers.map(
                      (
                        approver: {
                          userId: number;
                          userFullName: string;
                        ...
                        <HStack cursor="pointer" role="group" key={index}>
                          <UserInfo
                            ...
                            roleStyle={{ fontStyle: 'italic', mt: '4px' }}
                          />
                          <Icon
                            as={CloseIcon}
                            boxSize="16px"
                    ...
                    {shouldEdit &&
                      (hasMultipleApprover ||
                        (!hasMultipleApprover &&
                          form.values.levels[data.levelNumber - 1].approvers
                            ?.length === 0)) && (
                        <IconButton
                          variant="solid"
                          bgColor="#F1F1F1"
            ...
            {typeof errors?.levels?.[data.levelNumber - 1] === 'object' &&
