...
const NoteDetails = (props: NoteFormModalProps) => {
  const { isOpen, onClose, defaultNote } = props;
  const {
  ...
  const {
    ...
    { skip: !parsedData?.contextId }
  );

  const note = useMemo(() => {
    if (parsedData?.contextId) {
  ...
  return (
    <>
      ...
      >
        ...
        <ModalBody p={0} m={0} width="full" minH="inherit">
            ...
            >
              ...
              >
                ...
                <HStack
                  ...
                  spacing="16px"
                >
                  <Flex display={noteDetail ? 'none' : 'flex'}>
