...
interface useMaintenanceBudgetReportTable {
  search?: string;
  customPageSize?: number;
}

const useMaintenanceBudgetReportTable = (
  props: useMaintenanceBudgetReportTable
) => {
  const { search, customPageSize } = props;
  const [pageNumber, setPageNumber] = useState(1);
  const [pageSize, setPageSize] = useState(DEFAULT_PAGE_SIZE);

  const { data, isLoading, isFetching } = useGetMaintenanceBudgetReportQuery({
...
export default useMaintenanceBudgetReportTable;
